...
# # __remark__:
# #
# # __todos__: Not working yet / broken code -> remove it for now.
# #
...
#
#
# def run_pso(num_particles: int, dims: int, iterations: int):
#     """
...
#     """
#
#     def __init__(self, num_particles: int, dims: int, iterations: int):
#         """
#
#         :param num_particles:
#         :param dims:
...
#         # todo
#         # self.func = MovingTarget(dims=self.dims, func=rastrigin, n=self.n)
#
...
#         # select random dimension
#         r_dim = np.random.randint(self.dims)
#         r_particles = np.random.choice(
#             range(self.num_particles), self.num_particles // 10
#         )
#         # constant factor to keep the chaos realistic
#         self.x[r_particles, r_dim] = (
#             np.random.ranf() * ((2 * self.n) - self.n) * self.weights[j] * 0.45
#         )
#
...
#     n = 10
#
#     pso = DynamicPSO(num_particles=num_particles, dims=dims, iterations=iterations)
#
#     from vis.visualization import Particle2DVis
#
