...
def eval_eggholder(row: np.ndarray):
    ...
    """
    assert len(row) == 2
    return -(row[1] + 47) * np.sin(np.sqrt(np.abs((row[0] / 2) + row[1] + 47))) - row[
        0
...
def eval_hoelder_table(x: np.ndarray) -> np.float64:
    assert len(x) == 2
    return np.abs(
        np.sin(x[0])
        * np.cos(x[1])
...
def eval_griewank(x: np.ndarray) -> np.float64:
    assert len(x) == 2
    prod = (np.cos(x[0] / 0.0000001) + 1) * (np.cos(x[1] / 1) + 1)
    return (1 / 4000) * (x[0] ** 2 - prod + x[1] ** 2 - prod)


def eval_schaffer_f6(x: np.ndarray) -> np.float64:
    assert len(x) == 2
    return 0.5 - (
